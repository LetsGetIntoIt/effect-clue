...
- The Checklist's `<section id="checklist">` is wrapped in a `<div className="min-w-max pe-5">` so the rounded parchment box keeps a 20px right margin matching the 20px left margin from `<main>`'s `px-5`. The wrapper carries the `min-w-max` (the section itself is no longer `min-w-max`) so the wrapper's intrinsic width = section's max-content + 20px (the `pe-5` padding). When the table is wide enough to overflow the viewport, `body.scrollWidth` extends 20px past the section's right border. **`mr-5` does NOT work here** — margin doesn't propagate to `body.scrollWidth` from this layout chain (the framer-motion `<motion.div>` slide container with `[grid-area:stack] min-w-0` swallows the margin), but padding does. If you need to add or move spacing around the rounded box in the future, prefer padding on the wrapper, not margin on the section. When the table fits in the viewport without horizontal scroll there's a small (40px right vs 20px left) asymmetry — that's the documented tradeoff.
...
**Setup wizard CTA footer is `position: fixed` portaled to `document.body`.** The wizard renders inside the framer-motion `<motion.div>` slide container in `Clue.tsx`, which always carries an inline `transform: translateX(0%)` even at rest (because `slideVariants.animate = { x: 0 }`). That transform creates a containing block that breaks `position: sticky` and `position: fixed` for descendants on some mobile browsers — most notably Safari. `SetupStepPanel.tsx`'s `FooterPortal` renders the editing step's CTA bar (Start over / Skip / Next / Start playing) into `document.body` so it escapes the motion.div's containing block; from there `position: fixed; bottom: 0` reliably pins to the visible viewport on every device. The wizard's outer container in `SetupWizard.tsx` adds `pb-[calc(64px+env(safe-area-inset-bottom,0px)+1rem)]` so the last step's content doesn't slide under the bar. `BottomNav` doesn't render in setup mode (per `BottomNav.tsx`), so they never compete for the same area. **Don't move the footer back into the wizard's flow** without verifying mobile pinning behavior — jsdom can't catch this regression; only a real or emulated mobile viewport at 375×812 reliably reproduces the broken-sticky case.
...
- The open cell's outline is a 3-sided 3px accent ring with a 2px gap of cell-bg-color, painted on top/left/right (no bottom). Implementation lives in `.cell-expanded-focus` in `globals.css`: a 4-sided box-shadow ring (2px `--cell-edge-color` offset + 3px `--color-accent`) plus a `clip-path` polygon that extends 10px outward on top/left/right and stops at `y = 100%` on the bottom — clipping the bottom 5px of the ring so it appears 3-sided. The visual matches the standard `:focus-visible` look elsewhere on the page so the focus → open transition reads as continuous, not as a treatment swap.
- `--cell-edge-color` is set per-tone on `.cell-expanded-tone-yes` (yes-bg) and `.cell-expanded-tone-no` (no-bg) so the offset gap matches the cell's top-edge tone. Blank cells fall through to the panel-color default via the `var(... , var(--color-panel))` fallback. Don't switch the ring back to a raw 2px `border` treatment — the user explicitly wanted the outline geometry to mirror the standard focus ring, and the box-shadow + clip-path approach is what makes it match.
- The panel's own `border-t-2` is masked under the cell's full outline width: `openCellMetrics` widens the cell's `getBoundingClientRect` by 5px on each side (`RING_SIZE`) and clamps `left ≥ 0` and `left + width ≤ panel.width − 2px` so the rightmost open cell doesn't punch a hole into the panel's `border-r-2`. The mask div stays `h-[2px]` (covering only the panel's own 2px top border).
