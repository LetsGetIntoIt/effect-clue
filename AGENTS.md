...
- The explanation panel's accent frame is split across two `<td>`s in the panel row. The content `<td>`'s inner `<div>` carries `border-t/r/b-[3px] border-accent` but **no left border**; the blank first-column `<td>` to its left carries the matching `border-t/l/b-[3px] border-accent` (no right border). The two meet at the seam with no border between them, so the 3px accent box fully encloses the row from its left edge around to its right. The blank cell is the row-label column, so it always lines up with the panel above/below it — the border is constant, no measurement. (Pre-fix the blank cell had only a 1px gray `border-b` and the panel's left side was open, so the frame looked unclosed on the left.)
...
**Per-mode gate state.** Each tour's `effect-clue.tour.{screenKey}.v1` entry holds per-mode subkeys (`normal` / `teach`) — each carries its own `lastVisitedAt` / `lastDismissedAt` and its own 4-week clock. A user who walked the normal-mode tour still gets the teach-mode tour on their first teach-mode visit and vice versa. Pre-teach-me records lift forward: a flat v1 record becomes `{ normal: <flat> }` so existing gate state survives the upgrade. The shape lives in `src/ui/tour/TourState.ts`; `useTourGate(screen, mode, …)` reads the active subkey, and `pickFirstEligibleScreenKey(candidates, mode, now)` / `findHighestPriorityEligibleTour(mode, now)` route the coordinator's precedence walk through it.

**Delta tour on mode switch.** When the user has dismissed/completed the tour in the OTHER mode within the re-engage window but the CURRENT mode is fresh, `useTourGate` returns `{ shouldShow: true, deltaMode: true }`. `TourProvider`'s `filterStepsByTeachMode` then keeps only steps whose `requiredTeachMode === currentMode`, dropping the shared structural steps the user already walked. The popover renders just the mode-specific differences (e.g. teach-mode users see the cell-explanation Y/N picker step + the global Check callout instead of the whole DEDUCTIONS / LEADS / HYPOTHESIS chain).

**Mode-aware body copy.** Step config supports optional `titleKeyTeachMode` / `bodyKeyTeachMode` next-intl key overrides. When the user is in teach-me mode AND the override is set, `TourPopover` renders that key instead of the base `titleKey` / `bodyKey`. Used today for `suggest.addForm` ("Log every suggestion — the solver checks your work against it") and `firstSuggestion.checklist` ("Mark what you learned"). The override lookup is mode-only (no viewport conditional); per-viewport copy still goes through `sideByViewport` / `anchorByViewport`. Steps that need different copy in teach-mode add the two override keys to `messages/en.json` alongside the base keys.
...
- **`checklistSuggest`** — fires on first visit to the Play pane (Checklist + Suggest). Launch: dismiss `setup` first (e.g. seed `effect-clue.tour.setup.v1` with `lastDismissedAt`), then load `/play?view=checklist`. Step count is viewport-conditional, mode-conditional, AND delta-conditional. In normal mode the registry filter yields 13 (desktop) / 14 (mobile) steps; in teach mode the filter drops the LEADS / HYPOTHESIS / DEDUCTIONS / My Cards / refute steps and adds the teach-mode cell-explanation step plus the global Check callout. In delta mode the registry filter keeps ONLY steps whose `requiredTeachMode === currentMode`, dropping the shared structural steps the user already walked in the other mode. Verify the "N of M" counter matches what's visible at each viewport × mode × delta combination.
- **`firstSuggestion`** — fires once per 4-week window per mode when the user logs the first suggestion of any session. Each mode tracks its own clock, so a user who already saw the normal-mode variant still gets the teach-mode variant the first time they log a suggestion in teach-mode. Launch: clear all tour state, dismiss splash + the per-screen tours, set up a game with default players, navigate to suggest mode, and submit the form. The popover fires immediately after the suggestion is added. Step count is viewport-conditional: 1 on desktop (the "see updated deductions" callout, since both panes are visible side-by-side), 2 on mobile (tap-Checklist to navigate to the Checklist pane, then the "see updated deductions" callout against the case-file there). Body copy is mode-aware via `bodyKeyTeachMode`.
- **`sharing`** — follow-up tour that opens the overflow menu and walks the three menu items inside it: Invite a player, Continue on another device, My card packs. Each step force-opens the menu via `forceOpenOverflowMenu: true` (decoupled from `anchor === "overflow-menu"` so the popover anchors to the specific menu item while the menu stays open). Has prerequisites: both the `setup` AND `checklistSuggest` tours must have been dismissed first IN EITHER MODE (the prereq check is mode-agnostic — a user who walked the prereqs in normal-mode shouldn't be blocked from sharing in teach-mode). Launch: seed `effect-clue.tour.setup.v1` AND `effect-clue.tour.checklistSuggest.v1` with `lastDismissedAt` (in either mode), leave `effect-clue.tour.sharing.v1` unseeded, then load `/play?view=setup`. Does NOT redirect off other screens.
...
- **"Click" vs "tap"** — pick the verb that matches the device input. The user does not "click" a phone screen and does not "tap" a desktop mouse; using the wrong word feels off and gives away that the app is rendering generic copy. Use `useHasKeyboard()` (`src/ui/hooks/useHasKeyboard.ts`) to detect — `true` means hover + fine pointer (mouse / trackpad), `false` means touch. Two patterns are already in place:
  - **For dynamic UI copy**, branch on the hook and pick one of two i18n keys. The convention is to suffix the key with the input type: `revealHintMouse` / `revealHintTouch`, `pressEnterToEditMouse` / `pressEnterToEditTouch`, etc. This avoids placeholder gymnastics in the i18n string and keeps each language file natural.
  - **For tour bodies that take an action**, use the `{action}` placeholder in the i18n value (e.g. `"<strong>{action} the highlighted cell to open the breakdown.</strong>"`); the `TourPopover` already wires `useHasKeyboard()` to `verbClick` / `verbTap` in the `onboarding` namespace and passes the resolved verb as `t.rich`'s `action` argument.

  This applies to verbs anywhere in user-facing copy — buttons that say "Click to … / Tap to …", help text that describes how to interact with a cell, error messages that suggest a fix, banner reveal prompts. Internal code (component names, comment prose, variable names) can use either word; the rule is about what the user reads. Strings that are mobile-only by construction (e.g. `priorRowHintMobile: "Tap to edit"` — only rendered on the mobile prior-log) can write "tap" inline without the branch.

Apply all four rules wherever the user is reading copy (i18n strings, error messages, tooltips, tour text). Internal variable names can keep `owner` / `ownership` as a technical term (the codebase uses the `Owner` type widely) — those names predate this rule and renaming the type for a vocabulary nicety is more churn than it's worth.
