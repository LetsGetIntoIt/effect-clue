 ...
 *     by the current user.
 *   - `saveCardPack({ clientGeneratedId, label, cardSet })` —
...
const encodeCardSet = (cardSet: CardSet): string => JSON.stringify(cardSet);
...
export async function saveCardPack(
    ...
    const cardSetData = encodeCardSet(input.cardSet);
    return withServerAction(
        Effect.gen(function* () {
            const sql = yield* PgClient.PgClient;
            const rows = yield* sql<{
 ...
 * label, cardSet)` payloads is a no-op. The conflict policy
