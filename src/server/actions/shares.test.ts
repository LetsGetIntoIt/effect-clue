...
const SAMPLE_SOLVER_MODE = JSON.stringify(true);
...
const projectInsert = (id: string, input: unknown): RecordedInsert => {
    ...
    return {
        ...
        solverModeData:
            kind === "transfer"
                ? ((obj["solverModeData"] as string | undefined) ?? null)
                : null,
    };
};
...
describe("createShare — universal sign-in", () => {
    ...
    test("anonymous-plugin user + transfer share → ERR_SIGN_IN_REQUIRED", async () => {
        ...
        const result = await callCreateShare({
            ...
            solverModeData: SAMPLE_SOLVER_MODE,
        });
        expect(result).toBeInstanceOf(Error);
        expect((result as Error).message).toContain(
            "sign_in_required_to_share",
...
describe("createShare — kind dispatch (signed-in)", () => {
    ...
    test("kind: transfer → all twelve columns populated, including teach-mode", async () => {
        ...
        expect(insert.solverModeData).toBe(SAMPLE_SOLVER_MODE);
    });
